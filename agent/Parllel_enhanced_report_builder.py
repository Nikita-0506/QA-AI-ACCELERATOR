"""Enhanced report builder with detailed analysis."""
import hashlib
import json
import os
...
class EnhancedReportBuilder:
    ...
    def _prioritize_regressions(self, regressions: List[Regression]) -> List[Regression]:
        ...
        Priority scoring:
        - Has error message: +1000 points
        - Duration increase: +1 point per millisecond
        
        Returns regressions sorted by priority (highest first).
        """
        def calculate_priority(regression: Regression) -> float:
            ...
            if regression.current_duration_ms and regression.baseline_duration_ms:
                ...
                score += max(0, duration_increase)  # Only count increases
    ...
    def _analyze_regressions_optimized(
        ...
        This method:
        1. Checks cache for each regression
        2. Identifies which need new analysis
        3. Runs those analyses in parallel
        4. Combines cached and new results
        """
        if not regressions:
            return []
        ...
        print(f"  ✓ Cache hit: {len(cached_results)}/{len(regressions)} ({len(cached_results)*100//len(regressions)}%)")
    ...
    def _get_cache_key(self, regression_dict: Dict[str, Any], key_changes: Any) -> str:
        ...
        cache_input = {
            ...
            "changes": str(key_changes)[:500]  # Limit changes to avoid huge keys
        }
    ...
    def _analyze_parallel(
        self,
        to_analyze: List[tuple],  # List of (index, regression_dict)
        key_changes: Any,
        file_change_summaries: List[Dict[str, Any]]
    ) -> List[tuple]:  # List of (index, result_dict)
        """Execute LLM analyses in parallel using ThreadPoolExecutor."""
        
        def analyze_single(item):
            """Analyze a single regression."""
